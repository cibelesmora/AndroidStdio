...
def login():
    ...
    print("u=",u)
    if u and u.password == request.form["password"]:Responsemsg="Correct Credentials"
    elif u and u.password != request.form["password"]:Responsemsg="Wrong Password"
    elif not u:Responsemsg="User Does not Exist"
...
def getaccounts():
    ...
    if not u or u.password != request.form["password"]:
        ...
        for i in a:
          if (i.type=='from'):from_acc=i.account_number;
        for j in a:
          if (i.type=='to'):to_acc=i.account_number;
...
def changepassword():
    ...
    if not u:
        Responsemsg="Error"
    else:Responsemsg="Change Password Successful"; u.password = newpassword; db_session.commit()
